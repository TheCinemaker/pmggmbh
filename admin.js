...
function openWeeklyReportModal(report, workWeek) {
  ...
  const tableHeader = workWeek.map((day, index) => `<th>${dayNames[index]}<br><small>${day.day}.</small></th>`).join('');
  const tableRows = report
    .filter(user => user.isMissing)
    .map(user => {
      const cells = workWeek.map(day => {
        if (user.weekStatus[day.day] === undefined) { return `<td class="status-na"></td>`; }
      ...
      return `<tr><td>${user.name}</td>${cells}<td>${action}</td></tr>`;
    }).join('');
  const modalContent = `<div class="modal large" role="dialog" aria-modal="true"><div class="modal-header"><h4>Wochenbericht (Nur fehlende Einträge)</h4><button class="modal-close" aria-label="${DE.close}"><svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M18.3 5.71a1 1 0 0 0-1.41 0L12 10.59 7.11 5.7A1 1 0 1 0 5.7 7.11L10.59 12l-4.9 4.89a1 1 0 1 0 1.41 1.42L12 13.41l4.89 4.9a1 1 0 0 0 1.42-1.41L13.41 12l4.9-4.89a1 1 0 0 0-.01-1.4Z"/></svg></button></div><div class="modal-body"><div class="report-table-wrapper"><table class="report-table"><thead><tr><th>Mitarbeiter</th>${tableHeader}<th>Aktion</th></tr></thead><tbody>${tableRows || `<tr><td colspan="7">Keine fehlenden Einträge für diese Woche gefunden!</td></tr>`}</tbody></table></div></div><div class="modal-footer"><button class="modal-primary">${DE.close}</button></div></div>`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (weeklyReportBtn) {
    weeklyReportBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (!response.ok) {
            const errBody = await response.json().catch(() => ({ message: 'A riport generálása sikertelen.' }));
  ...
  document.addEventListener('keydown', (e) => { const isMac = navigator.platform.toUpperCase().includes('MAC'); const key = (e.key || '').toLowerCase(); if ((isMac && e.metaKey && key === 'r') || (!isMac && e.ctrlKey && key === 'r')) { e.preventDefault(); refreshBtn?.click(); } });
  
  const backToTopButton = document.getElementById('backToTopBtn');
  if (backToTopButton) { window.addEventListener('scroll', () => { if (window.scrollY > 300) { backToTopButton.classList.add('show'); } else { backToTopButton.classList.remove('show'); } }); backToTopButton.addEventListener('click', () => { window.scrollTo({ top: 0, behavior: 'smooth' }); }); }
  
  const startAutoUpdate = () => { if (autoUpdateInterval) return; (async () => { try { await doLoad(true); } catch (e) { console.error('Auto-update first run error:', e); } })(); console.log(`Auto-update started (every ${AUTO_UPDATE_MS / 1000 / 60} min).`); autoUpdateInterval = setInterval(async () => { console.log('Auto-update tick…'); try { await doLoad(true); } catch (e) { console.error('Auto-update tick error:', e); } }, AUTO_UPDATE_MS); };
  const stopAutoUpdate = () => { if (!autoUpdateInterval) return; console.log('Auto-update stopped.'); clearInterval(autoUpdateInterval); autoUpdateInterval = null; };
  if (autoUpdateToggle) { const isEnabled = localStorage.getItem('autoUpdateEnabled') === 'true'; autoUpdateToggle.checked = isEnabled; if (isEnabled) startAutoUpdate(); autoUpdateToggle.addEventListener('change', () => { if (autoUpdateToggle.checked) { startAutoUpdate(); localStorage.setItem('autoUpdateEnabled', 'true'); } else { stopAutoUpdate(); localStorage.setItem('autoUpdateEnabled', 'false'); } }); document.addEventListener('visibilitychange', () => { const enabled = autoUpdateToggle.checked; if (document.hidden) { stopAutoUpdate(); } else if (enabled) { startAutoUpdate(); } }); }
});
